...
export default function ValidationTextFields() {
  ...
  /*   const onSubmitHandler = (e) => {
    e.preventDefault();
    axios.put(`http://localhost:3001/api/collab/${id}`, collaborateur)
      .then((res) => {  
  ...
  const onSubmitHandler = (e) => {
    ...
    axios
      .put(`http://localhost:3001/api/collab/${id}`, UpdateCollab)
      .then((res) => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3001/api/collab/${id}`)
      .then((res) => {
        const {
          Email,
          Nom,
      ...
      <FormControl>
          ...
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <DemoContainer components={['DatePicker']}>
              <TextField
                ...
                helperText={
                  errors.NumeroGsmPersonnel ? (
                    <>{errors.NumeroGsmPersonnel}</>
                  ) : (
                    'Veulliez Entrer Votre N° GSM Personnel'
                  )
                }
              />
              <TextField
            ...
            className="form-control"
              ...
              <DatePicker
                ...
                slotProps={{
                  textField: {
                    helperText: errors.DateValiditePermis ? (
                      <>{errors.DateValiditePermis}</>
                    ) : (
                      'Veuillez entrer votre date de validité du permis de conduire'
                    ),
                  },
                }}
                style={{ marginBottom: '16px' }}
          ...
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <DemoContainer components={['DatePicker']}>
              ...
              <DatePicker
                ...
                slotProps={{
                  textField: {
                    helperText: errors.DateValiditéCIN ? (
                      <>{errors.DateValiditéCIN}</>
                    ) : (
                      'Veulliez Entrer Votre Date validité CIN'
                    ),
                  },
                }}
              />
              ...
              <DatePicker
                ...
                slotProps={{
                  textField: {
                    helperText: errors.DateValiditéPassport ? (
                      <>{errors.DateValiditéPassport}</>
                    ) : (
        ...
        <Stack direction="row" spacing={2} /* onSubmit={CreateUser} */>
          <Button variant="contained" endIcon={<SendIcon />} onClick={onSubmitHandler}>
            {' '}
            {/* component={RouterLink} to={`http://localhost:3000/dashboard/collaborateur`} */}
            Modifier Collaborateur
          </Button>
          <Button variant="contained" component={RouterLink} to="/dashboard/collaborateur">
            Annuler
          </Button>
        </Stack>
      </FormControl>
