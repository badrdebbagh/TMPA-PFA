...
export default function EnhancedTable() {
  ...
  const onSubmitHandler = (e) => {
    e.preventDefault();
    Axios.post('http://localhost:3001/api/Infractions', form)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    Axios.get('http://localhost:3001/api/Infractions').then((res) => {
      setInfractions(res.data);
    });
    const itemCount = infractions.length;
    if (itemCount > 0) {
  ...
  const CreateUser = () => {
    Axios.post('http://localhost:3001/api/Infractions', {
      lastname,
      firstname,
      email,
      phone,
  ...
  const OnDelete = (id) => {
    if (window.confirm('Are you sure to delete this infraction?')) {
      Axios.delete(`http://localhost:3001/api/Infractions/${id}`)
        .then((res) => {
          console.log(res.data);
          setMessage(res.data.message);
          setShow(true);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = infractions.map((n) => n._id);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - infractions.length) : 0;

  const visibleRows = React.useMemo(
    () =>
      stableSort(infractions, getComparator(order, orderBy)).slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage
      ),
    [order, orderBy, page, rowsPerPage, infractions]
  );
          ...
          <TableContainer>
            <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle" size={dense ? 'small' : 'medium'}>
              <EnhancedTableHead
                ...
                rowCount={infractions.length}
              />
              <TableBody>
                {visibleRows.map((infractions, index) => {
                  const isItemSelected = isSelected(infractions._id);
                  const labelId = `enhanced-table-checkbox-${index}`;

                  return (
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, infractions._id)}
                      role="checkbox"
                    ...
                    >
                      <TableCell padding="checkbox">
                        <Checkbox
                          ...
                          inputProps={{
                            'aria-labelledby': infractions._id,
                          }}
                        />
                      </TableCell>
                      <TableCell component="th" _id={infractions._id} scope="row" padding="none">
                        {infractions.declarationAuteur}
                      </TableCell>
                      <TableCell align="right">{infractions.numeroDeclaration}</TableCell>
                      <TableCell align="right">{infractions.immatriculationVehicule}</TableCell>
                      <TableCell align="right">{infractions.dateHeureInfraction}</TableCell>
                      ...
                      <TableCell align="right">
                        <Button
                          ...
                          to={`http://localhost:3000/dashboard/editInfractions/${infractions._id}`}
                          endIcon={<EditIcon />}
                        >
                          Modifier
                        </Button>{' '}
                        <Button endIcon={<DeleteIcon />} onClick={() => OnDelete(infractions._id)}>
                          Archiver
                        </Button>
                      </TableCell>
                    </TableRow>
